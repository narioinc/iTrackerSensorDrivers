...
u32_t nrf_drv_lis3dh_write_reg(u8_t reg, u8_t data)
{
    uint32_t err_code;
		err_code = itracker_i2c_write(LIS3DH_ADDR, reg, &data, 1);
    return err_code;
}


u8_t nrf_drv_lis3dh_read_reg(u8_t reg, u8_t * p_data)
{
    uint32_t err_code;
		err_code = itracker_i2c_read(LIS3DH_ADDR, reg, p_data, 1);
    return err_code;
}
